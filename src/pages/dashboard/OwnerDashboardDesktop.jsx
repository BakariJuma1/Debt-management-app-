...
function StatCard({ title, value, icon, trend, change, secondaryValue, variant = 'default' }) {
  ...
  return (
    <div className={`p-6 rounded-xl shadow-sm border ${variantClasses[variant]} ${
...
function BusinessOwnerDashboard() {
  ...
  const [businessName, setBusinessName] = useState('My Business'); // Default name, replace with actual business name
  ...
  async function fetchDashboardData() {
    ...
    try {
      ...
      // If business name is included in response:
      ...
      <div className="flex-1 p-8 ml-64 mt-18">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8 gap-4">
          <div>
            ...
            <p className="text-gray-600">Comprehensive overview of {businessName}</p>
          </div>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full border border-gray-300 rounded-lg px-3 py-2"
                    defaultValue={`${businessName} Debt Report`}
                  />
                </div>
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full border border-gray-300 rounded-lg px-3 py-2"
                    rows="3"
                    defaultValue={`Please find attached the latest debt report for ${businessName}.`}
                  />
                </div>
              </div>
          ...
          <ErrorBoundary>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 mb-8">
              ...
              <StatCard 
                ...
                icon={<FiTrendingUp className={
                  dashboardData.summary?.recovery_rate > 75 ? "text-green-500" : 
              ...
              <StatCard 
                ...
                value={dashboardData.summary?.avg_repayment_days || 'N/A'} 
                icon={<FiClock className={
                  dashboardData.summary?.avg_repayment_days < 30 ? "text-green-500" : 
                  dashboardData.summary?.avg_repayment_days < 60 ? "text-amber-500" : "text-red-500"
                } />}
                trend={dashboardData.summary?.avg_repayment_days < 30 ? 'down' : 
                      dashboardData.summary?.avg_repayment_days < 60 ? 'none' : 'up'}
                variant={
                  dashboardData.summary?.avg_repayment_days < 30 ? 'success' : 
                  dashboardData.summary?.avg_repayment_days < 60 ? 'warning' : 'danger'
                }
              />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 space-y-6">
                ...
                <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                  <div className="flex justify-between items-center mb-4">
                    <h2 className="text-lg font-semibold text-gray-800">{businessName} Customer Segmentation</h2>
              ...
              <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
  <div className="flex justify-between items-center mb-4">
    <h2 className="text-lg font-semibold text-gray-800">{businessName} Debt Composition</h2>
    ...
    <div>
      ...
      <ul className="space-y-2">
        {dashboardData.customer_segmentation?.top_debtors?.length > 0 ? (
          ...
          <li className="text-gray-500">No debtor data</li>
        )}
      </ul>
    </div>
  </div>
              ...
              <div className="space-y-6">
                ...
                <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
  <h2 className="text-lg font-semibold text-gray-800 mb-4">{businessName} Customer Credit Scores</h2>
                    ...
                    {dashboardData.summary ? (
                      <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
                          <Pie
                            data={[
                              { name: "Paid", value: dashboardData.summary.paid_debts || 0 },
                              { name: "Unpaid", value: dashboardData.summary.unpaid_debts || 0 },
                              { name: "Overdue", value: dashboardData.summary.overdue_debts || 0 }
                            ]}
                            dataKey="value"
                            nameKey="name"
                            outerRadius={80}
