...
import {
  ...
  FiHome, FiBarChart2, FiCreditCard, FiMessageSquare
} from "react-icons/fi";
import {
...
const MobileOwnerDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      const url = queryString ? `${API_BASE_URL}/dashboard-owner?${queryString}` : `${API_BASE_URL}/dashboard-owner`;
  ...
  const renderOverviewTab = () => (
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-2xl shadow-sm p-4 border border-gray-100">
        <h2 className="text-md font-semibold text-gray-800 mb-3">Business Performance</h2>
        {dashboardData.performance_vs_target ? (
          <>
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs text-gray-600">Target: Ksh {dashboardData.performance_vs_target.target_amount?.toLocaleString() || 'N/A'}</span>
  ...
  const renderBusinessTab = () => (
    <div className="space-y-4">
        ...
        <div className="space-y-3">
          {dashboardData.customer_segmentation?.top_debtors?.slice(0, 5).map((customer, index) => (
            <div key={index} className="p-3 bg-gray-50 rounded-lg">
              <div className="flex justify-between items-start">
                <div>
                  <p className="text-sm font-medium text-gray-900">{customer.customer || 'Unknown'}</p>
                  <p className="text-xs text-gray-500 mt-1">{customer.phone || 'N/A'}</p>
                </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm p-4 border border-gray-100">
        <div className="flex justify-between items-center mb-3">
          <h2 className="text-md font-semibold text-gray-800">Upcoming Payments</h2>
          <button className="text-xs text-indigo-600 font-medium flex items-center">
            View All <FiChevronRight className="ml-1" />
          </button>
        </div>
  ...
  const renderActionsTab = () => (
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-2xl shadow-sm p-4 border border-gray-100">
        ...
        <div className="grid grid-cols-2 gap-3">
          <button className="flex flex-col items-center justify-center p-3 bg-indigo-50 rounded-xl hover:bg-indigo-100 transition-colors">
            ...
            <span className="text-xs text-indigo-600 font-medium text-center">Send Report</span>
          </button>
          <button className="flex flex-col items-center justify-center p-3 bg-emerald-50 rounded-xl hover:bg-emerald-100 transition-colors">
            ...
            <span className="text-xs text-emerald-600 font-medium text-center">Add Staff</span>
          </button>
          <button className="flex flex-col items-center justify-center p-3 bg-amber-50 rounded-xl hover:bg-amber-100 transition-colors">
            ...
            <span className="text-xs text-amber-600 font-medium text-center">View Reports</span>
          </button>
          <button className="flex flex-col items-center justify-center p-3 bg-purple-50 rounded-xl hover:bg-purple-100 transition-colors">
            ...
            <span className="text-xs text-purple-600 font-medium text-center">Business Settings</span>
          </button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm p-4 border border-gray-100">
        ...
        <p className="text-sm text-gray-600 mb-3">Export your business data for reporting or analysis</p>
        <button 
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-16">
        ...
        {dashboardData ? (
          <div>
            ...
            <div className="flex border-b border-gray-200 mb-4">
              ...
              <button 
                className={`flex-1 py-2 text-center text-sm font-medium ${activeTab === 'business' ? 'text-indigo-600 border-b-2 border-indigo-600' : 'text-gray-500'}`}
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 flex justify-around py-2">
        ...
        <button 
          className={`flex flex-col items-center px-4 py-1 rounded-lg ${activeTab === 'business' ? 'bg-indigo-50 text-indigo-600' : 'text-gray-500'}`}
          onClick={() => setActiveTab('business')}
        >
